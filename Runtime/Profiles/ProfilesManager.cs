...
namespace SaveSystemPackage.Profiles {
  ...
  public class ProfilesManager {
    ...
    private readonly Dictionary<string, string> _profilesMap;
    private bool _performed;

    internal ProfilesManager(ProfilesManagerData data) {
      _profilesMap = data.profilesMap ?? new Dictionary<string, string>();
    }
    ...
    public SaveProfile CreateProfile([NotNull] string name, string iconId = null) {
      ...
      } while (_profilesMap.ContainsKey(id));
      ...
      _profilesMap.Add(id, name);
      var managerData = new ProfilesManagerData { profilesMap = new Map<string, string>(_profilesMap) };
    ...
    public async IAsyncEnumerable<SaveProfile> LoadProfiles() {
      foreach (string profileId in _profilesMap.Keys) {
    ...
    internal void ThrowIfProfileExistsWithName(string name) {
      if (_profilesMap.ContainsValue(name))
    ...
    private async Task SaveData(ProfileData profileData, ProfilesManagerData managerData) {
      ...
      try {
        await SaveSystem.Game.SaveData(SaveSystem.KeyProvider.Provide<ProfileData>(profileData.id), profileData);
        await SaveSystem.Game.SaveData(SaveSystem.KeyProvider.Provide<ProfilesManagerData>(), managerData);
      }
    ...
    private async Task DeleteProfileData(SaveProfile profile, ProfilesManagerData managerData) {
      ...
      try {
        await SaveSystem.Game.DeleteData<ProfileData>(SaveSystem.KeyProvider.Provide<ProfileData>(profile.Id));
        await SaveSystem.Game.SaveData(SaveSystem.KeyProvider.Provide<ProfilesManagerData>(), managerData);
